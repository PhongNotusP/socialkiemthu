...
describe('Test POST /api/friends/decline/:idSender', () => {
    ...
    it('Can decline friend request', async () => {
        const response = await request(app).
            post('/api/friends/decline/' + idUser1)
    ...
    it('Cannot decline friend with user who didnt send request for you', async () => {
        const response = await request(app).
            post('/api/friends/decline/' + idUser3)
            .set({ Authorization: 'Bearer ' + token2 })
            .send({});
        const { status, body } = response;
        const { success, data, message } = body;
    ...
    it('Cannot decline youself', async () => {
        const response = await request(app).
            post('/api/friends/decline/' + idUser2)
            .set({ Authorization: 'Bearer ' + token2 })
            .send({});
        const { status, body } = response;
        const { success, data, message } = body;
